...
# This script creates ICs by sampling the original test's density field
# instead of distributing particles on a uniform grid in order to
# emulate cells.
# ---------------------------------------------------------------------
...
if __name__ == "__main__":
    ...
    # new gas particle mass
    m = mp.sum() / nparts ** 3

    # allocate some memory space
    pos = np.zeros((int(N * 1.5), 3))

    # compute the number of particle per cell
    Ni = density * V / m

    # get the integer part
    Nieff = (np.floor(Ni)).astype(int)

    # get the remaining fraction
    Nifra = Ni - Nieff

    # max number of part per cell
    Nmax = Ni.max()

    # generate particles for the fractional part (Monte Carlo)
    rnd = rng.random(N)
    cNi = rnd < Nifra

    # generate the coordinates
    x = (rng.random(N) - 0.5) * dx + xp[:, 0]
    y = (rng.random(N) - 0.5) * dx + xp[:, 1]
    z = (rng.random(N) - 0.5) * dx + xp[:, 2]
    posi = np.transpose([x, y, z])

    # keep only the non zero
    posi = posi[cNi]

    # and store those in pos
    j = posi.shape[0]
    pos[0:j] = posi
    ...
    for i in range(N):
        ...
        if np.fmod(i, 100000) == 0:
            print(i, N)

        if Nieff[i] > 0:

            x = (rng.random(Nieff[i]) - 0.5) * dx + xp[i, 0]
            y = (rng.random(Nieff[i]) - 0.5) * dx + xp[i, 1]
            z = (rng.random(Nieff[i]) - 0.5) * dx + xp[i, 2]
            posi = np.transpose([x, y, z])
    ...
    w.write("ilievTest4Sampled.hdf5")
