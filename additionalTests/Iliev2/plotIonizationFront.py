...
def plot_ionization_fronts_from_snapshots(snaplist):
    """
    ...
    # plot actual values
    ax1.plot(times_trec, r_ifront_ana.to("kpc"), label="ana")
    ax1.plot(times_trec, r_ifront.to("kpc"), label="GEARRT")
    if skip_last:
        times_trec = times_trec[:-1]
        v_ifront = v_ifront[:-1]
        v_ifront_ana = v_ifront_ana[:-1]
    ax2.plot(times_trec, v_ifront_ana.to("kpc/kyr"), label="ana")
    ax2.plot(times_trec, v_ifront.to("kpc/kyr"), label="GEARRT")
    ax1.set_ylabel("$r_I$ [kpc]")
    ax2.set_ylabel("$v_I$ [kpc/kyr]")
    #-------------------------------------------------------------------------
    
    # plot ratio
    #  ax1.plot(times_trec, r_ifront / r_ifront_ana)
    #  if skip_last:
    #      times_trec = times_trec[:-1]
    #      v_ifront = v_ifront[:-1]
    #      v_ifront_ana = v_ifront_ana[:-1]
    #
    #  ax2.plot(times_trec, v_ifront / v_ifront_ana)
    #
    ...
    for ax in fig.axes:
        ax.set_xlabel("$t/t_{rec}$")


#  def plot_ionization_fronts_from_log():
    """
    ...
    ax1.plot(times/t_rec, rI.to("kpc"), ls="--")

    dr = rI[1:] - rI[:-1]
    dr_bins = r_bin_centers[i] - r_bin_centers[i-1] 
    print(dr_bins)
    print(dr_bins/length_units)
    dt = times[1:] - times[:-1]
    vI = dr/dt
    ax2.plot(times[1:]/t_rec, vI, ls="--")
...
if __name__ == "__main__":
    ...
    plot_ionization_fronts_from_snapshots(snaplist)
    #  plot_ionization_fronts_from_log()
