...
void simulation_params_print(struct units *units, struct simulation_params *params) {
  ...
  message("%22s: %.6e", "approx dt [internal units]", conversions_estimate_dt(params));
  message("%22s: %.6e", "approx dt [s]             ", conversions_estimate_dt(params) / units->time_units);
  message("%22s: %.6e", "approx dt [kyr]           ", conversions_estimate_dt(params) * units->time_units / const_yr * 1e-3);
}
