...
def parse_args():
    ...
    parser.add_argument("-f", "--file", help="file to data", default="analytic.dat")
...
if __name__ in ("__main__"):
    ...
    args = parse_args()
    file = args.file.lower()

    if file in ("f", "finlator", "fin"):
        a, E, Erho = extract_data(finlatorfile)
        savename = "finlator.png"
    elif file in ("a", "analytic", "ana"):
        a, E, Erho = extract_data(analyticfile)
        savename = "analytic.png"
    else:
        print("Argument not understood!")
        exit(1)

    # Scaled expected solutions
    expected_E = fit_func(a, -1, 1.0)
    expected_Erho = fit_func(a, -4, 1.0)

    expected_E = expected_E / expected_E[0] * E[0]
    expected_Erho = expected_Erho / expected_Erho[0] * Erho[0]
    ...
    # Plot photon energy
    ax1.plot(a, E, zorder=2)  # , **plot_kwargs)
    ax1.plot(
        a, expected_E, label="Analytic $\propto a^{-1}$", **analytic_solution_kwargs
    )
    ...
    ax2.plot(a, Erho, zorder=2)  # , **plot_kwargs)
    ax2.plot(
        a, expected_Erho, label="Analytic $\propto a^{-4}$", **analytic_solution_kwargs
    )
