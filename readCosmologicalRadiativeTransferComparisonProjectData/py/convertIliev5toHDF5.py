...
quantity_descriptions = ["neutral hydrogen fraction", "ionized hydrogen fraction", "pressure", "temperature", "mach number", "gas number density"]
...
def make_h5file(code):
    ...
    for age in times:
        ...
        for q, quantity in enumerate(quantities):
            ...
            dss = slice_age.create_dataset(quantity, (ncells, ncells,), dtype="f", compression="gzip")
            dss[:] = sdata[:]
            ...
            dsp = profile_age.create_dataset(quantity, (ncells,), dtype="f", compression="gzip")
            dsp_std = profile_age.create_dataset(quantity + "_std", (ncells,), dtype="f", compression="gzip")
            dsp[:] = pdata[:]
            dsp_std[:] = pstd[:] 
            ...
            dsp_std.attrs["description"] = quantity_descriptions[q] + " standard deviation"
